...
#	 * @since 20170111
	 ...
	 *
	 * @since 20170113
	 * @var object  TCC_Plugin_Paths object.
	 */
	public $paths = null;
	/**
	 *
	 ...
	 */
	public function get_stylesheet( $file = 'css/tcc-plugin.css' ) {
	...
	public function settings_link( $links, $file, $data, $context ) {
		if ( strpos( $file, basename( $this->paths->file ) ) ) {
			...
			if ( is_plugin_active( $file ) ) {
				...
				if ( $url ) $links['settings'] = sprintf( '<a href="%s"> %s </a>', esc_url( $url ), esc_html__( 'Settings', 'tcc-plugin' ) );
			}
		}
		return $links;
	}
