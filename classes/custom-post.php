...
abstract class RC_Custom_Post_Type {
  ...
  protected function translate_post_count($count) {
    return esc_html_nx('%1$s %2$s by this author','%1$s %2$s by this author',$count,'first placeholder is numeric, second should be a noun','tcc-fluid');
  }

  protected function translated_text() {
    static $text;
    if (empty($text)) {
      $text =  array('404'       => _x('No %s found',          'placeholder is plural form',  'tcc-fluid'),
